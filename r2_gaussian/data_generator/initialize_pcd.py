    ...
    if args.evaluate:
        with torch.no_grad():
            ...
            if vol_gt.shape != vol_pred.shape:
                target_shape = vol_pred.shape
                if any(v > t for v, t in zip(vol_gt.shape, target_shape)):
                    raise ValueError(
                        f"vol_gt shape {tuple(vol_gt.shape)} exceeds target {tuple(target_shape)}"
                    )
                padded = torch.full(
                    target_shape, 0.0, device=vol_gt.device, dtype=vol_gt.dtype
                )
                start = [(t - v) // 2 for t, v in zip(target_shape, vol_gt.shape)]
                end = [s + v for s, v in zip(start, vol_gt.shape)]
                padded[
